...
st.info("This is a fresh build to run the sportsbook scraper and display the raw JSON output.")
...
def run_scraper_and_get_json(actor_id):
    ...
    st.write("Starting sportsbook scraper...")
...
if st.button("Run Scraper"):
    ...
    if error:
        ...
        st.warning("Scraper ran but returned no data.")
    else:
        st.success("Scraping complete! Raw JSON output is displayed below.")
        st.json(raw_json_data)
        st.header("Next Step: Analyze the JSON")
        st.info("The next step is to analyze the JSON structure above to build a real parser function.")
